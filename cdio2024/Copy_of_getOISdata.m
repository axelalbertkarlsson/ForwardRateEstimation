...
usedInstr = false(length(times), 27);
...
for k=1:length(times(1:TT)) 
   ...
   cf_dates = cell(length(indInstr),1);
   for j = 1:length(indInstr) 
       cf_dates{j} = instr.data{j}.cfDatesFix;
   end

   removeIdx = (indInstr == remove_assetNo);
   indInstr(removeIdx) = [];
   marketQuoteP(removeIdx) = [];
   instrID(removeIdx) = [];
...
save OISdata fH zH yieldH CAll D Eeig ef eonioa FAll I_xAll I_zAll n_cAll oAll priceAll year_fracAll TT times indInstrAll usedInstr