...
% for method = (methods)
%     [discountFactors, iterations, F, f_f] = NewtonRaphson(yOIS_Actual, delta_T, T_values, tol,maxIter, nodes, P0, last_entries, method);
%     for i = 1:5 
%         f_f(:,i) = f_f(:,6);
%     end
...
% day2 = 22;
% IntPrice = priceAll{day,1};
% IntPrice2 = IntPrice([1:21, 23:end]); %loocv
% T_values = T_valuesAll{day};
% T_values2 = T_values([1:21, 23:end]); %loocv
% last_entries = T_valuesEnd{day};
% last_entries2 = last_entries([1:21, 23:end]); %loocv
% % T = last_entries;
% T = last_entries2; %loocv
% % nodes = last_entries;
% nodes = last_entries2;%loocv
% % yOIS_Actual = IntPrice(1:end); % test
% yOIS_Actual = IntPrice2(1:end); 
% tol = 10^-10;
% maxIter = 10000;
% short_rate = yOIS_Actual(1);
% P0 = exp(-short_rate * (1 / 365));
% 
% for index = 1:length(T_values2)
%     % Get the current cell's value
%     T = T_values{index};
%     if isempty(T)
%        % Handle empty cells by assigning an empty array
...
%     end
% end
% delta_T = delta_T([1:21, 23:end]); %loocv
% methods = ["linear_discount", "linear_spot", "raw", "log_rates"];
% for method = (methods)
%     [discountFactors, iterations, F, f_f_loocv] = NewtonRaphson(yOIS_Actual, delta_T, T_values2, tol, maxIter, nodes, P0, last_entries2, method);
%     for i = 1:5 
%         f_f_loocv(:,i) = f_f_loocv(:,6);
%     end
%     % plot_f_f(f_f_loocv(1:3650), method);
% end
%% Find price
% test = exp(-discountFactors.*last_entries2');
% summa = zeros(size(f_f_loocv));
% for index = 1:length(f_f_loocv)
%     summa(index) = sum(f_f_loocv(1:index));
% end
% %summa = summa * 1 /365;
% test2 = exp(-summa);
%% 3D plots
...
% y_error1 = zeros(4826, 26);
% for i = 1:4826
%     IntPrice = priceAll{i,1};
%     IntPrice2 = IntPrice([1:2, 4:end]);
%     % T = last_entries;
%     T = T_valuesEnd{i};
%     yOIS_Actual = IntPrice2(1:end);
%     tol = 10^-10;
...
%         end
%     end
%     delta_T = delta_T([1:2, 4:end]); %loocv
%     last_entries = T_valuesEnd{i};
%     last_entries2 = last_entries([1:2, 4:end]);
%     nodes = last_entries2;
%     [discountFactors, iterations, F, f_f, y_error] = NewtonRaphson(yOIS_Actual, delta_T, T_values2, tol, maxIter, nodes, P0, last_entries2, method);
%     f_toti(i, :) = f_f(1:3650);
%     y_error1(i,:) = y_error;
% end
% for i = 1:5
...
% %error = mean(abs(y_error1(:,25)))
% 
% %% new test forward
% yOIS_Actual = priceAll{4826,1};
% zeroYields2 = zeros(4826,28);
% for i = 1:4826
%     last_entries = T_valuesEnd{i};
%     ftest = f_toti(i,:); % Daily forward rates (1x3650)
%     N = length(ftest);   % Number of days (should be 3650)
%     deltaTdaily = (1/365) * ones(N,1); % Daily day count fractions (assuming ACT/365)
% 
%     % Initialize Discount Factors for All Days
%     D = zeros(N,1); 
% 
%     % Compute Discount Factor for Day 1
%     D(1) = 1 / (1 + ftest(1) * deltaTdaily(1));
% 
%     % Compute Discount Factors for Days 2 to N
%     for j = 2:N
%         D(j) = D(j-1) / (1 + ftest(j) * deltaTdaily(j));
%     end
% 
%     % ====== Step 2: Compute Discount Factors for the 28 Maturities ======
% 
%     % Initialize vectors to store Discount Factors and Zero Yields for 28 maturities
%     numMaturities = length(last_entries); % Should be 28
%     D_28 = zeros(numMaturities,1);        % Discount factors for 28 maturities
%     zeroYields = zeros(numMaturities,1);  % Zero-coupon yields for 28 maturities
% 
%     % Loop through each maturity to compute D_28 and zeroYields
%     for k = 1:numMaturities
%         % Extract the k-th maturity in years
%         T_k = last_entries(k); % Alternatively, use T_28x1(k) if different
% 
%         % Convert maturity in years to day index
%         dayIndex = round(T_k * 365);
% 
%         % Handle cases where dayIndex exceeds available data
%         if dayIndex > N
%             warning('Maturity %.2f years (day index %d) exceeds available forward rate data (%d days).', ...
%                     T_k, dayIndex, N);
%             % Option 1: Use the last available discount factor
%             D_28(k) = D(N);
% 
%             % Option 2: Alternatively, you can extrapolate or handle as needed
%             % For simplicity, we'll use Option 1 here
%         elseif dayIndex < 1
%             warning('Maturity %.2f years results in invalid day index (%d). Setting D_28(k) to 1.', ...
%                     T_k, dayIndex);
%             D_28(k) = 1;
%         else
%             % Assign the discount factor corresponding to the dayIndex
%             D_28(k) = D(dayIndex);
%         end
% 
%         % Compute the zero-coupon yield (continuously compounded)
%         % Ensure T_k is not zero to avoid division by zero
%         if T_k > 0
%             zeroYields(k) = -log(D_28(k)) / T_k;
%         else
%             warning('Maturity is zero for k=%d. Setting zeroYield to NaN.', k);
%             zeroYields(k) = NaN;
%         end
%     end
%     zeroYields2(i,:) = zeroYields;
% end
% errorsquared = zeros(4826,1);
% for i = 1:4826
%     yOIS_Actual = priceAll{i,1};
%     errorsquared(i) = (y_error1(i, 3) + (zeroYields2(i,3)- yOIS_Actual(3)))^2;
% end
% final_error = mean(errorsquared)
% 
% final_matrix_to_carl(:,3) = errorsquared;

test9 = zeros(4826,28);
for i = 1:4826
    yOIS_Actual = priceAll{i,1};
    test9(i,:) = ((zeroYields2(i,:)' - yOIS_Actual(:))).^2;
end
test10 = mean(test9(:,3));
% ====== Step 3: Display the Results ======

% fprintf('Maturity (Years)\tDiscount Factor\tZero Yield (%%)\n');
% for k = 1:numMaturities
%     fprintf('%.2f\t\t%.6f\t\t%.4f%%\n', last_entries(k), D_28(k), zeroYields(k)*100);
% end
% disp("fon")

% delta_t = 1 / 365; % Daily interval
% num_days = length(f_toti); % Total number of days
% P_t_T = zeros(1, num_days); % Array to store bond prices
% P_t_T_loocv = zeros(num_days, num_days); % LOOCV bond prices matrix
% cumulative_sum = 0;
% for i = 1:num_days
%     cumulative_sum = cumulative_sum + f_toti(i) * delta_t; % Sum up forward rates
%     P_t_T(i) = exp(-cumulative_sum); % Calculate bond price
% end
% for excluded_index = 1:num_days
%     % Leave out one forward rate
%     f_f_loocv_temp = f_toti;
%     f_f_loocv_temp(excluded_index) = NaN; % Mark as excluded
%     % Recalculate bond prices without the excluded forward rate
%     cumulative_sum_temp = 0;
%     for i = 1:num_days
%         if ~isnan(f_f_loocv_temp(i)) % Skip excluded index
%         cumulative_sum_temp = cumulative_sum_temp + f_f_loocv_temp(i) * delta_t;
%         P_t_T_loocv(excluded_index, i) = exp(-cumulative_sum_temp); % Update bond price
%         else
%         P_t_T_loocv(excluded_index, i) = NaN; % Mark as invalid for LOOCV
%         end
%     end
% end
% LOOCV_errors = zeros(1, num_days);
% for i = 1:num_days
%     valid_indices = ~isnan(P_t_T_loocv(i, :));
%     predicted_prices = P_t_T_loocv(i, valid_indices);
%     actual_prices = P_t_T(valid_indices);
%     LOOCV_errors(i) = norm(predicted_prices - actual_prices) / norm(actual_prices); % Relative error
% end


% delta_t = 1 / 365; % Daily interval
% num_days = length(f_f_loocv); % Total number of days
% P_t_T = zeros(1, num_days); % Array to store bond prices
% P_t_T_loocv = zeros(num_days, num_days); % LOOCV bond prices matrix
% cumulative_sum = 0;
% for i = 1:num_days
%     cumulative_sum = cumulative_sum + f_f_loocv(i) * delta_t; % Sum up forward rates
%     P_t_T(i) = exp(-cumulative_sum); % Calculate bond price
% end
% for excluded_index = 1:num_days
%     % Leave out one forward rate
%     f_f_loocv_temp = f_f_loocv;
%     f_f_loocv_temp(excluded_index) = NaN; % Mark as excluded
%     % Recalculate bond prices without the excluded forward rate
%     cumulative_sum_temp = 0;
%     for i = 1:num_days
...
%     LOOCV_errors(i) = norm(predicted_prices - actual_prices) / norm(actual_prices); % Relative error
% end
% Total_error = mean(LOOCV_errors);
% test16 = sum(LOOCV_errors);
    ...
    for iter = 1:maxIter
        ...
        if norm(Delta_x,2) < tol
            ...
            if method == "linear_discount"
                ...
                y_error = yOIS_Actual(1:end - 1)-yOIS;
            elseif method == "raw"
...
function J = Compute_Jacobian(P, delta_T, discountFactors, T_values, nodes, num_variables, P0, method)
    ...
    %globalTenors = nodes;
    J = zeros(27, 27);  % Preallocate Jacobian matrix
