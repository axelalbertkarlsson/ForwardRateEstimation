...
addpath('C:\Users\axela\TATA62\cdio2024')
addpath('C:\Users\axela\TATA62\cdio2024\measurement')
...
T_valuesAll = cell(length(times), 1);
T_valuesEnd = cell(length(times), 1);
...
% fprintf('%s: %s -> %s\n', datestr(times(k)), datestr(rfrTimeLimit), datestr(rfrTime));
  ...
  last_entries = cellfun(@(x) x(end), T_values, 'UniformOutput', true); % Axeltest
    ...
    %Kanske relevant senare
    % for j = 1:length(T_values)
    % T = T_values{j}; % Get the current array of cash flow times
    % if length(T) > 1
    %     T(1:end-1) = T(1:end-1) + 4 / 365; % Add 4 days to all but the last element
    % end
    % T_values{j} = T; % Update the cell with the adjusted times
    % end
